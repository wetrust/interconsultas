...
class PartosModel
{
    public static function getAllPartos($solicitud_profesionalemail)
    {
        ...
        if (Session::get('user_account_type') == 2){
            $sql = "SELECT partos.parto_id, partos.solicitud_id, solicitudes.solicitud_nombre, solicitudes.solicitud_apellido, solicitudes.solicitud_rut, solicitudes.solicitud_ciudad, solicitudes.solicitud_lugar, solicitudes.solicitud_fum,partos.fecha_parto, partos.semanas, partos.dias, partos.peso, partos.talla, partos.imc, partos.estado_nutricional, partos.etnia, partos.paridad, partos.lugar, partos.pesofetal, partos.tallafetal, partos.craneofetal, partos.apgar_uno, partos.apgar_cinco, partos.sexo, partos.meconio, partos.ipn, partos.peso_eg, partos.peso_eg_estado, partos.ipn_eg, partos.ipn_eg_estado, partos.comentarios,partos.hipoglicemia,partos.alta, partos.protocolo_hipoglicemia FROM partos INNER JOIN solicitudes ON partos.solicitud_id = solicitudes.solicitud_id where solicitudes.solicitud_email = :solicitud_profesionalemail";
        }
        else{
            $sql = "SELECT partos.parto_id, partos.solicitud_id, solicitudes.solicitud_nombre, solicitudes.solicitud_apellido, solicitudes.solicitud_rut, solicitudes.solicitud_ciudad, solicitudes.solicitud_lugar, solicitudes.solicitud_fum,partos.fecha_parto, partos.semanas, partos.dias, partos.peso, partos.talla, partos.imc, partos.estado_nutricional, partos.etnia, partos.paridad, partos.lugar, partos.pesofetal, partos.tallafetal, partos.craneofetal, partos.apgar_uno, partos.apgar_cinco, partos.sexo, partos.meconio, partos.ipn, partos.peso_eg, partos.peso_eg_estado, partos.ipn_eg, partos.ipn_eg_estado, partos.comentarios,partos.hipoglicemia,partos.alta, partos.protocolo_hipoglicemia FROM partos INNER JOIN solicitudes ON partos.solicitud_id = solicitudes.solicitud_id where solicitudes.solicitud_profesionalemail = :solicitud_profesionalemail";
        }
    ...
    {
        $database = DatabaseFactory::getFactory()->getConnection();
        $sql = "SELECT parto_id, solicitud_id, fecha_parto, semanas, dias, peso, talla, imc, estado_nutricional, etnia, paridad, lugar, pesofetal, tallafetal, craneofetal, apgar_uno, apgar_cinco, sexo, meconio, ipn, peso_eg, peso_eg_estado, ipn_eg, ipn_eg_estado, comentarios,hipoglicemia,alta,protocolo_hipoglicemia, edad_materna, pesoegcorregido, pesoegcorregidoestado, factores, rnsintomatico, factoresjson,rn, otras FROM partos WHERE solicitud_id = :solicitud_id LIMIT 1";
        $query = $database->prepare($sql);
    ...
    public static function createPartos($solicitud_id, $fecha_parto, $semanas, $dias, $peso, $talla, $imc, $estado_nutricional, $etnia, $paridad, $lugar, $pesofetal, $tallafetal, $craneofetal, $apgar_uno, $apgar_cinco, $sexo, $meconio, $ipn, $peso_eg, $peso_eg_estado, $ipn_eg, $ipn_eg_estado, $comentarios, $hipoglicemia, $alta, $protocolo_hipoglicemia, $edad_materna, $pesoegcorregido, $pesoegcorregidoestado)
    {
        ...
        $sql = "INSERT INTO partos (solicitud_id, fecha_parto, semanas, dias, peso, talla, imc, estado_nutricional, etnia, paridad, lugar, pesofetal, tallafetal, craneofetal, apgar_uno, apgar_cinco, sexo, meconio, ipn, peso_eg, peso_eg_estado, ipn_eg, ipn_eg_estado, comentarios,hipoglicemia,alta, protocolo_hipoglicemia, edad_materna, pesoegcorregido, pesoegcorregidoestado) VALUES (:solicitud_id,:fecha_parto,:semanas,:dias,:peso,:talla,:imc,:estado_nutricional,:etnia,:paridad,:lugar,:pesofetal,:tallafetal,:craneofetal,:apgar_uno,:apgar_cinco,:sexo,:meconio,:ipn,:peso_eg,:peso_eg_estado,:ipn_eg,:ipn_eg_estado,:comentarios,:hipoglicemia,:alta, :protocolo_hipoglicemia, :edad_materna, :pesoegcorregido, :pesoegcorregidoestado)";
        $query = $database->prepare($sql);
        $query->execute(array(':solicitud_id' => $solicitud_id, ':fecha_parto' => $fecha_parto,':semanas' => $semanas,':dias' => $dias,':peso' => $peso,':talla' => $talla,':imc' => $imc,':estado_nutricional' => $estado_nutricional,':etnia' => $etnia,':paridad' => $paridad,':lugar' => $lugar,':pesofetal' => $pesofetal,':tallafetal' => $tallafetal,':craneofetal' => $craneofetal,':apgar_uno' => $apgar_uno,':apgar_cinco' => $apgar_cinco,':sexo' => $sexo,':meconio' => $meconio,':ipn' => $ipn,':peso_eg' => $peso_eg,':peso_eg_estado' => $peso_eg_estado,':ipn_eg' => $ipn_eg,':ipn_eg_estado' => $ipn_eg_estado,':comentarios' => $comentarios,':hipoglicemia' => $hipoglicemia,':alta' => $alta, ':protocolo_hipoglicemia' => $protocolo_hipoglicemia, ':edad_materna' => $edad_materna, ':pesoegcorregido' => $pesoegcorregido, ':pesoegcorregidoestado' => $pesoegcorregidoestado));
    ...
    public static function updatePartos($parto_id, $peso, $talla, $imc, $estado_nutricional, $etnia, $paridad, $lugar, $pesofetal, $tallafetal, $craneofetal, $apgar_uno, $apgar_cinco, $sexo, $meconio, $ipn, $peso_eg, $peso_eg_estado, $ipn_eg, $ipn_eg_estado, $comentarios, $hipoglicemia, $alta, $protocolo_hipoglicemia, $edad_materna, $pesoegcorregido, $pesoegcorregidoestado) {
        $database = DatabaseFactory::getFactory()->getConnection();
        $sql = "UPDATE partos SET peso = :peso, talla = :talla, imc = :imc, estado_nutricional = :estado_nutricional, etnia = :etnia, paridad = :paridad, lugar = :lugar, pesofetal = :pesofetal, tallafetal = :tallafetal, craneofetal = :craneofetal, apgar_uno = :apgar_uno, apgar_cinco = :apgar_cinco, sexo = :sexo, meconio = :meconio, ipn = :ipn, peso_eg = :peso_eg, peso_eg_estado = :peso_eg_estado, ipn_eg = :ipn_eg, ipn_eg_estado = :ipn_eg_estado, comentarios = :comentarios, hipoglicemia = :hipoglicemia, alta = :alta, protocolo_hipoglicemia = :protocolo_hipoglicemia, edad_materna = :edad_materna, pesoegcorregido = :pesoegcorregido, pesoegcorregidoestado = :pesoegcorregidoestado WHERE parto_id = :parto_id";
        $query = $database->prepare($sql);
