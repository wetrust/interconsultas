...
class PartosModel
{
    ...
    {
        $database = DatabaseFactory::getFactory()->getConnection();
        $sql = "SELECT parto_id, solicitud_id, fecha_parto, semanas, dias, peso, talla, imc, estado_nutricional, etnia, paridad, lugar, pesofetal, tallafetal, craneofetal, apgar_uno, apgar_cinco, sexo, meconio, ipn, peso_eg, peso_eg_estado, ipn_eg, ipn_eg_estado, comentarios,hipoglicemia, edad_materna, pesoegcorregido, pesoegcorregidoestado, factores, rnsintomatico, factoresjson,rn, otras FROM partos WHERE solicitud_id = :solicitud_id LIMIT 1";
        $query = $database->prepare($sql);
    ...
    public static function updatePartos($parto_id, $peso, $talla, $imc, $estado_nutricional, $etnia, $paridad, $lugar, $pesofetal, $tallafetal, $craneofetal, $apgar_uno, $apgar_cinco, $sexo, $meconio, $ipn, $peso_eg, $peso_eg_estado, $ipn_eg, $ipn_eg_estado, $comentarios, $hipoglicemia, $rn, $factores, $edad_materna, $pesoegcorregido, $pesoegcorregidoestado) {
        $database = DatabaseFactory::getFactory()->getConnection();
        $sql = "UPDATE partos SET peso = :peso, talla = :talla, imc = :imc, estado_nutricional = :estado_nutricional, etnia = :etnia, paridad = :paridad, lugar = :lugar, pesofetal = :pesofetal, tallafetal = :tallafetal, craneofetal = :craneofetal, apgar_uno = :apgar_uno, apgar_cinco = :apgar_cinco, sexo = :sexo, meconio = :meconio, ipn = :ipn, peso_eg = :peso_eg, peso_eg_estado = :peso_eg_estado, ipn_eg = :ipn_eg, ipn_eg_estado = :ipn_eg_estado, comentarios = :comentarios, hipoglicemia = :hipoglicemia, rn = :rn, factores = :factores, edad_materna = :edad_materna, pesoegcorregido = :pesoegcorregido, pesoegcorregidoestado = :pesoegcorregidoestado WHERE parto_id = :parto_id";
        $query = $database->prepare($sql);
