<?php
    ...
    if (Session::get("user_account_type") == 4) {
        ...
        $this->pdf->Ln(1);
        $html = '<table><tbody><tr><td>Fecha solicitud de la ecografía</td><td>: '.$solicitud_fecha.'</td><td>FUR referida o corregida</td><td>: '.$solicitud_fum.'</td></tr></tbody></table>';
        $this->pdf->writeHTMLCell('', '', '', '', $html, 0, 1, 0, true, 'J', true);
        $this->pdf->Ln(1);
        ...
        $this->pdf->writeHTMLCell('', '', '', '', $html, 0, 1, 0, true, 'J', true);
        $this->pdf->Ln(1);
        $html = '<table><tbody><tr><td>Edad Gestacional: '.htmlentities($this->solicitud->solicitud_egestacional).'</td><td></td></tr></tbody></table>';
        $this->pdf->writeHTMLCell('', '', '', '', $html, 0, 1, 0, true, 'J', true);
        $this->pdf->Ln(1);
    ...
    if ($this->respuesta_anatomia_segundo == "de aspecto general normal"){
        $html = '<table><tbody><tr><td>Atrio posterior: '. htmlentities($this->respuesta_atrio_posterior) .' '.$this->respuesta_atrio_posterior_mm.'</td><td>Cerebelo: '.htmlentities($this->respuesta_cerebelo_text).' '.$this->respuesta_cerebelo.'</td><td>Cist. magna: '.htmlentities($this->respuesta_cisterna_m).' '.$this->respuesta_cisterna_m_mm.'</td></tr></tbody></table>';
        $this->pdf->writeHTMLCell('', '', '', '', $html, 0, 1, 0, true, 'J', true);
        $this->pdf->Ln(1);
    }
