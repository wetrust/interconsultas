<?php
    ...
    $this->pdf->writeHTMLCell('', '', '', '', $html, 0, 1, 0, true, 'J', true);
    $this->pdf->Ln(1);
    $html = '<table><tbody><tr><td>Frecuencia cardiaca fetal (FCF): '.htmlentities($this->respuesta_fcf).'</td><td>Sexo: '.htmlentities($this->respuesta_sexo_fetal).'</td><td>Placenta: '.htmlentities($this->respuesta_placenta).', '.$this->respuesta_placenta_insercion.'</td></tr></tbody></table>';
    $this->pdf->writeHTMLCell('', '', '', '', $html, 0, 1, 0, true, 'J', true);
    $this->pdf->Ln(1);
