<?php
    ...
    $html = '<table><tbody><tr><td>Edad Gestacional al exámen: '. htmlentities($this->respuesta_eg) .'</td><td>Feto en presentación: '.htmlentities($this->respuesta_presentacion).'</td><td>Dorso Fetal: '.htmlentities($this->respuesta_dorso_segundo).'</td></tr></tbody></table>';
    $this->pdf->writeHTMLCell('', '', '', '', $html, 0, 1, 0, true, 'J', true);
    $this->pdf->Ln(1);
    $html = '<table><tbody><tr><td>Líquido amniótico: '.htmlentities($this->respuesta_liquido_amniotico).'</td><td>BVM: '.htmlentities($this->respuesta_bvm).'</td><td>Placenta: '. $this->respuesta_placenta. ', '.$this->respuesta_placenta_insercion.'</td><td>Sexo: '.htmlentities($this->respuesta_sexo_fetal).'</td></tr></tbody></table>';
    $this->pdf->writeHTMLCell('', '', '', '', $html, 0, 1, 0, true, 'J', true);
    $this->pdf->Ln(1);
