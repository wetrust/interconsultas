<?php
    ...
    $this->pdf->Ln(1);
    $html = '<table><tbody><tr><td>Nombre: '.htmlentities($this->paciente->solicitud_nombre).'</td><td>Sexo: '.htmlentities($this->parto->sexo).'</td></tr></tbody></table>';
    $this->pdf->writeHTMLCell('', '', '', '', $html, 0, 1, 0, true, 'J', true);
    $this->pdf->Ln(1);
    ...
    $this->pdf->writeHTMLCell('', '', '', '', $html, 0, 1, 0, true, 'J', true);
    $this->pdf->Ln(1);
    $html = '<table><tbody><tr><td>IMC Materno (peso * talla)&#94;2: '.htmlentities($this->parto->imc).'</td><td>IPN: '.htmlentities($this->parto->ipn).' [((peso*talla)^3) * 100]</td></tr></tbody></table>';
    $this->pdf->writeHTMLCell('', '', '', '', $html, 0, 1, 0, true, 'J', true);
    $this->pdf->Ln(1);
    $html = '<table><tbody><tr><td>Tipo de parto: '.htmlentities($this->parto->tipo).'</td><td>Peso / Ege: pct. '.htmlentities($this->parto->peso_eg).', '.htmlentities($this->parto->peso_eg_estado).'</td></tr></tbody></table>';
    $this->pdf->writeHTMLCell('', '', '', '', $html, 0, 1, 0, true, 'J', true);
    $this->pdf->Ln(1);
