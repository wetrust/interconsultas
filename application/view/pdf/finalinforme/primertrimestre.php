<?php
    ...
    $html = '<h4 style="border-bottom:1px solid #000;">A- Formulario referencia para evaluación ecográfica de primer trimestre</h4>';
    ...
    if ($this->respuesta_saco_valor > 0){
        $html = '<table><tbody><tr><td>Saco Gestacional: '. $this->respuesta_saco_gestacional.'</td>Saco gestional promedio: '. $this->respuesta_saco_valor . ' mm.</tr></tbody></table>';
        $this->pdf->writeHTMLCell('', '', '', '', $html, 0, 1, 0, true, '', true);
        $this->pdf->Ln(1);
    }
    else{
    ...
    $html = '<table><tbody><tr><td>Embrión: '. $this->respuesta_embrion.' mm.</td></tr></tbody></table>';
    $this->pdf->writeHTMLCell('', '', '', '', $html, 0, 1, 0, true, '', true);
    $this->pdf->Ln(1);
